...
color_array <- c("chocolate1", "burlywood" , "blue", "green")

chr2_stat_data$index = 0:93
chr2_stat_data$pacbio = -10 * log10(chr2_stat_data$pacbio_error / chr2_stat_data$pacbio_total + 0.000000000000000000001)
chr2_stat_data$stat_p = -10 * log10(chr2_stat_data$stat_error / chr2_stat_data$stat_total + 0.000000000000000000001)
chr2_stat_data$deepconsensus = -10 * log10(chr2_stat_data$deep_error / chr2_stat_data$deep_total + 0.000000000000000000001)
chr2_stat_data$var_p = -10 * log10(chr2_stat_data$var_error / chr2_stat_data$var_total + 0.000000000000000000001)
...
df2 <- chr2_stat_data %>%
  dplyr::select(index, stat_p, stat_total) %>%
  gather(key = "method", value = "value", -index, -stat_total)

df3 <- chr2_stat_data %>%
  dplyr::select(index, var_p, var_total) %>%
  gather(key = "method", value = "value", -index, -var_total)
...
total_count = sum(df4$count)
df4$count = 100 * df4$count / total_count
df4[94, 2] = 15

newdf <- rbind(df2, df1)
newdf <- rbind(newdf, df3)
newdf <- rbind(newdf, df4
               )
# main graph
