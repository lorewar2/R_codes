...
data_for_R_chr2_stat_var$pacbio = -10 * log10(data_for_R_chr2_stat_var$pacbio_error / data_for_R_chr2_stat_var$pacbio_total + 0.0000000001)
data_for_R_chr2_stat_var$ml_qual = -10 * log10(data_for_R_chr2_stat_var$ml_error / data_for_R_chr2_stat_var$ml_total + 0.0000000001)
data_for_R_chr2_stat_var$topo_qual = -10 * log10(data_for_R_chr2_stat_var$topo_error / data_for_R_chr2_stat_var$topo_total + 0.0000000001)
data_for_R_chr2_stat_var$deepconsensus = -10 * log10(data_for_R_chr2_stat_var$deep_error / data_for_R_chr2_stat_var$deep_total + 0.00000000001)
data_for_R_chr2_stat_var$pacbio_index = 0:93
data_for_R_chr2_stat_var$topoqual = 0:93
data_for_R_chr2_stat_var$index = 0:93
data_for_R_chr2_stat_var$topoqual_base_count = rep(0, 94)
# binned for chr2_mlqual
count = 0
sum = 0
base_count = 0
for (i in 1:94) {
  if (!is.nan(data_for_R_chr2_stat_var[i, 10])) {
    count = count + 1
    sum = sum +  data_for_R_chr2_stat_var[i, 10]
    base_count = base_count +  data_for_R_chr2_stat_var[i, 3]
  }
  prev_result = 0
  if (i %% 3 == 0) {
    tets = sum / count
    ...
    data_for_R_chr2_stat_var[i - 2, 14] = result
    data_for_R_chr2_stat_var[i - 1, 14] = result
    data_for_R_chr2_stat_var[i, 14] = result
    data_for_R_chr2_stat_var[i - 2, 16] = base_count
    data_for_R_chr2_stat_var[i - 1, 16] = base_count
    data_for_R_chr2_stat_var[i, 16] = base_count
    base_count = 0
    sum = 0
    count = 0
  }
...
data_for_R_chr2_stat_var$pacbio_index = test

data_for_R_chr2_stat_var = data_for_R_chr2_stat_var %>% 
  rename(
    variable_rate = topoqual,
    )

# make a thin dataframe with all info
df1 <- data_for_R_chr2_stat_var %>%
  dplyr::select(index, pacbio, pacbio_total) %>%
  gather(key = "method", value = "value", -index, -pacbio_total)

df3 <- data_for_R_chr2_stat_var %>%
  dplyr::select(index, deepconsensus, deep_total) %>%
  gather(key = "method", value = "value", -index, -deep_total)

df2 <- data_for_R_chr2_stat_var %>%
  dplyr::select(pacbio_index, variable_rate, topoqual_base_count) %>%
